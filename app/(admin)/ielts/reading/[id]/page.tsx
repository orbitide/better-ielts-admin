...
import { fetchReadingTestById, fetchReadingSections } from '@/lib/api/ielts'
import type { ReadingSectionsPage } from '@/lib/api/ielts'
import { ReadingTestDetailShell } from '@/components/ielts/ReadingTestDetailShell'
import type { ReadingTestDetail, SetContext } from '@/lib/types/ielts'

const SECTIONS_PAGE_SIZE = 10

const ReadingTestDetailPage = () => {
  const { id } = useParams<{ id: string }>()
  const searchParams = useSearchParams()
  const [test, setTest] = useState<ReadingTestDetail | null>(null)
  const [sectionsPage, setSectionsPage] = useState<ReadingSectionsPage | null>(null)
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetchReadingSections(id, page, SECTIONS_PAGE_SIZE),
    ])
      .then(([t, sp]) => {
        setTest(t)
        setSectionsPage(sp)
...
export default ReadingTestDetailPage
